...
var (
	ecaLogger = logging.MustGetLogger("eca")
	// the ASN1 object identifier of the subject's role
...
func NewECA() *ECA {
	eca := &ECA{CA: NewCA("eca")}
	flogging.LoggingInit("eca")

	// read or create global symmetric encryption key
	{
		cooked := ""
		raw, err := ioutil.ReadFile(filepath.Join(eca.path, "obc.aes"))
		if err != nil {
			r := rand.Reader
			...
			err = ioutil.WriteFile(filepath.Join(eca.path, "obc.aes"), []byte(cooked), 0644)
			if err != nil {
				ecaLogger.Panic(err)
			}
		} else {
	...
	// read or create global ECDSA key pair for ECIES
	{
		...
		cooked, err := ioutil.ReadFile(filepath.Join(eca.path, "obc.ecies"))
		if err == nil {
			...
			if err := ioutil.WriteFile(filepath.Join(eca.path, "obc.ecies"), cooked, 0644); err != nil {
				ecaLogger.Panic(err)
			}
		}
		...
		eca.obcPub = pem.EncodeToMemory(&pem.Block{
...
func (eca *ECA) populateUsersTable() {
	// populate user table
	users := viper.GetStringMapString("idprovider.eca.users")
...
func (eca *ECA) populateAffiliationGroupsTable() {
	key := "idprovider.eca.affiliations"
...
// stop the ECA
func (eca *ECA) Stop() {
	ecaLogger.Info("Stopping ECA...")
	if eca.gRPCServer != nil {
		eca.gRPCServer.Stop()
	}

	if err := eca.CA.Stop(); err != nil {
		ecaLogger.Errorf("ECA stopping err: %v", err)
	} else {
		ecaLogger.Info("ECA stoped")
	}
}
