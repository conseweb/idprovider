...
func (aca *ACA) populateAttribute(tx *sql.Tx, attr *AttributePair) error {
	var count int
	err := tx.QueryRow(sqlite_count_attribute_by_id_name,
	...
	if count > 0 {
		_, err = tx.Exec(sqlite_update_attribute_by_id_name,
		...
		_, err = tx.Exec(sqlite_insert_attr,
...
func (aca *ACA) findAttribute(owner *AttributeOwner, attributeName string) (*AttributePair, error) {
	...
	err := aca.db.QueryRow(sqlite_count_attribute_by_id_name,
	...
	err = aca.db.QueryRow(sqlite_select_attribute_by_id_name,
