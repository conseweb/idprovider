...
func NewTCA(eca *ECA) *TCA {
	tca := &TCA{NewCA("tca"), eca, nil, nil, nil, nil}
...
func (tca *TCA) readHmacKey() error {
	var cooked string
	file := filepath.Join(tca.path, "tca.hmac")
	...
	if err != nil {
		...
		err = ioutil.WriteFile(file, []byte(cooked), 0644)
		if err != nil {
			tcaLogger.Panic(err)
		}
	} else {
...
func (tca *TCA) readRootPreKey() error {
	var cooked string
	rootPkFile := filepath.Join(tca.path, "root_pk.hmac")
	raw, err := ioutil.ReadFile(rootPkFile)
	if err != nil {
		key := make([]byte, RootPreKeySize)
		rand.Reader.Read(key)
		cooked = base64.StdEncoding.EncodeToString(key)

		err = ioutil.WriteFile(rootPkFile, []byte(cooked), 0644)
		if err != nil {
			tcaLogger.Panic(err)
		}
	} else {
...
func (tca *TCA) persistCertificateSet(enrollmentID string, timestamp int64, nonce []byte, kdfKey []byte) error {
	...
	if _, err = tca.db.Exec(sqlite_insert_tcertificate, enrollmentID, timestamp, nonce, kdfKey); err != nil {
		tcaLogger.Error(err)
	}
	return err
}
...
func (tca *TCA) retrieveCertificateSets(enrollmentID string) (*sql.Rows, error) {
	return tca.db.Query(sqlite_select_tcertificate_by_enrollID, enrollmentID)
}
